...
The root `sources.lock` pins the full source commit for every game package. `tools/source-lock.sh` creates depth-one detached checkouts and rejects the wrong origin, revision, or a dirty source tree. Both clone and generation recipes use this check. Update the lock in the same commit as any generator and generated-output changes caused by a source revision update.

`tools/package-finalization/` contains the game-neutral package contract used by every installed game. `tools/rgbds-technical-machines.mjs` resolves the authoritative 50-row TM order shared by the Generation I and Generation II RGBDS sources. `tools/gen2/` contains the stable source-format modules shared by the structurally similar Gold/Silver and Crystal projects. `tools/gen3/` contains the stable GBA source-format modules shared by Ruby/Sapphire and Emerald. Its world-topology module expands only declared cardinal roots and converts unplaced source connections into interior navigation edges. The package builders, public commands, dependency locks, and package-specific artifact tests remain under their package directories. Package-specific acquisition rules, source aliases, world roots, sprite behavior, and audits belong there. Keep source parsing, palette rules, rendering decisions, checklist ID construction, and exact source audits outside package finalization.

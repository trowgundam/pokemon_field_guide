...
const encounterType = method => {
  ...
  if (!type) throw new Error(`Yellow encounter method '${method}' is not classified.`);
  return type;
};
...
for (const file of fs.readdirSync(path.join(source, 'data/wild/maps')).filter(x => x.endsWith('.asm'))) {
  ...
  for (const line of lines) {
    ...
    if (mon) area.encounters.push({ species: title(mon[2]), speciesId: `SPECIES_${mon[2]}`, minLevel: +mon[1], maxLevel: +mon[1], chance: slotChances[slot++] ?? 0, method, type: encounterType(method), version: 'Both' });
  }}
}
...
for (const match of rodText.matchAll(/^\s*db\s+([A-Z0-9_]+)((?:,\s*[A-Z0-9_]+,\s*\d+){4})\s*$/gm)) {
  ...
  for (const [index, mon] of values.entries()) area.encounters.push({ species: title(mon[1]), speciesId: `SPECIES_${mon[1]}`, minLevel: +mon[2], maxLevel: +mon[2], chance: superRodChances[index], method: 'Super Rod', type: encounterType('Super Rod'), version: 'Both' });
}
...
const worldImage = await assets.map('WORLD_KANTO.png', target => sharp({ create: { width: worldWidth, height: worldHeight, channels: 4, background: { r: 40, g: 123, b: 101, alpha: 1 } } }).composite(
  placed.map(p => ({ input: maps.get(p.id).mapImage.localPath, left: p.x, top: p.y }))
).png().toFile(target));
const worlds = [{ id: 'yellow-kanto', image: worldImage, width: worldWidth, height: worldHeight, maps: placed }];
...
if (relevantAreas.reduce((sum, area) => sum + area.specialPokemon.length, 0) !== 39) throw new Error('Special Pokémon audit failed: expected 39 distinct acquisitions');
...
for (const area of relevantAreas) for (const version of ['Yellow']) {
...
return { source: 'pret/pokeyellow', generated: new Date().toISOString().slice(0, 10), areas: draftAreas, worlds, pokedex: dex, pokemonSprites, pokemonFallback };
} });
